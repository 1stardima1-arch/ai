...
1. Придумай ник и надёжный пароль (или сгенерируй — `openssl rand -base64 18`).
2. Добавь в `.env` (или Vercel → Environment Variables):
   ```
   ADMIN_USERNAME="твой-ник"
   ```
3. Запусти один раз (создаёт/обновляет пароль для этого ника):
   ```bash
   ADMIN_USERNAME="твой-ник" ADMIN_PASSWORD="твой-пароль" DATABASE_URL="<прод-строка>" npm run admin:provision
   ```
4. Заходи на `/login` под этим ником и паролем — появится «Сообщения
   студентов» в Профиле.

Сменить пароль позже — та же команда с новым `ADMIN_PASSWORD`.
